...
if __name__ == "__main__":
    ...
    response = requests.get(
        'https://data.rennesmetropole.fr/api/records/1.0/search/?dataset=etat-des-parcs-relais-du-reseau-star-en-temps-reel&facet=nom&facet=etat')
    data = response.json()
    print(data)
    print("######################### feature")
    ...
    for record in data['records']:
        ...
        maxsignal = 25
        signal_level = 1
        minetest_util.mark_totem(db, parkx, signal_level, parky, 2, 25, disporate, minetest_util.airnode,
        ...
        for y in range(boundingbox[1]):
            for z in range(boundingbox[2]):
                for x in range(boundingbox[0]):
                    ...
                    if vol[z][x][y]:
                        minetest_util.setblock(db, Pos(x + parkoffset[0], z + parkoffset[2], y + parkoffset[1]), minetest_util.airnode)
